---
title: "Plugins"
description: "Create and update plugins"
icon: "plug"
---

<Warning>
Plugins are not yet available for partners. They will be supported in a future developer console version.
</Warning>
...
[Plugins](/api-reference/plugins/create-a-new-plugin) for creating or managing Plugins

[PluginSubscriptions](/api-reference/plugins/subscribe-account-to-plugin) for assigning a Plugin to one or many Smokeball firms.
...
See [SDK Documentation](/docs/sdk-docs/675c9816cdaf6-getting-started) for usage information.
...
Smokeball firms are subscribed to plugins through the [PluginSubscriptions](/api-reference/plugins/subscribe-account-to-plugin) resource. Partners authorized through the Client Credentials Grant can then subscribe accounts to their plugins. Available plugins are scoped to API client ids, meaning that you can only subscribe to plugins that were created using the same API client id.
...
``` json
{
  ...
  "body": {
    ...
    "embedToken": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiYWRtaW4iOnRydWUsImlhdCI6MTUxNjIzOTAyMn0.KMUFsIDTnFmyG3nMiGM6H9FNFUROf3wh7SmqJp-QV30"
  }
}
```
...
Field | Description
------|------------
`accountId` | The unique identifier of the Smokeball firm (account) that the user belongs to.
`userId` | The unique identifier of the user who triggered the plugin.
`userEmail` | The email address of the user who triggered the plugin.
`matterId` | Optional unique identifier of the matter currently open. Only present when the plugin is opened within the context of a matter.
`embedToken` | Optional JWT token that can be exchanged for a [Smokeball access token via the SDK](/docs/sdk-docs/b4c6e12d3f9a0-authentication). Only present if the plugin is associated to an app ClientId.

Additional fields may also be added to this payload, depending on the source of the plugin.
