...
<CodeGroup>
  ```bash curl
    curl --request POST 'https://auth.smokeball.com/oauth2/token' \
      ...
      --data-urlencode 'scope=contacts/read matters/read matters/write'
  ```
  ```json http
  {
    "method": "POST",
...
<ParamField path="scope" type="string">
  ...
  OAuth 2.0 expects a client to state the scopes it wants, so we recommend always including this parameter.
  If it is omitted the authorization server currently falls back to issuing a token with all scopes assigned to the app, but you should not rely on this behaviour.
</ParamField>
