...
- **Authentication**: Increased the default refresh token expiry from 30 to 180 days, effective 24 August 2026. Applies to refresh tokens issued on or after that date.
- **BillingConfiguration**: Added a `PATCH` endpoint to update billing configuration.
- **Contacts**: Added `/contacts/batch` endpoint to fetch multiple contacts by ID.
- **Tasks**: Limit task notes to adhere to Smokeball app restrictions.
</Update>

<Update label="July 2026">
- **Events**: Deprecated `eventType` field in favor of new `type` field.
- **Files**: Added `matter`, `folder`, and download information to file search results.
- **Matters**: Added support for hidden items on the patch endpoint.
- **Tasks**: Fixed pagination `href` links on the `/tasks` endpoint to include all applied filters.
- **Tasks**: Added `dueDateOnly` and `completedDateOnly` fields; deprecated the previous date/time fields 'dueDate' and 'completedDate'.
</Update>

<Update label="May 2026">
- **Contacts**: Added `website` field.
- **Files**: Fixed intermittennt error when posting files to document sub-folders.
- **Matters**: Fixed duplicate empty role items being created when clearing a role via PATCH request.
- **MatterTypes**: Fixed webhooks to include missing `items` and `representativeOptions` fields.
- **Search**: Added advanced file/document searching functionality for matters.
</Update>

<Update label="April 2026">
- **Activities**: Fix incorrect zero default tax rate when not supplied.
- **BankAccounts**: Added matter balances endpoint.
- **Fees**: Added `CreatedDate` field to GET endpoints.
- **Folders**: Added `/path` endpoint to retrieve folder hierarchy for a specified folder.
- **Matters**: Added missing validation for staff.
</Update>

<Update label="March 2026">
- **Activities**: Added `InputTaxRate` and `OutputTaxRate` fields to activity codes.
- **Layouts**: Fixed errors updating events.
- **Staff**: Fixed incorrect paging total size.
</Update>

<Update label="February 2026">
- **BankAccounts**: Added requisition and fund protection functionality to request payment from trust.
- **Fees/Expenses**: Removed PATCH validation check for existing fee/expense records in API endpoints.
- **Invoices**: Added GET endpoint to retrieve individual invoices.
- **Staff**: Fixed missing `userId` field in webhook payloads.
- **Transactions**: Added create functionality for Trust accounts as well as ability to list transactions.
</Update>

<Update label="December 2025">
  - **Expenses**: Fix `outputTax` backwards compatibility issue for AU and UK regions. `outputTax` will now be set if missing from POST payload.
</Update>
...
- **FirmUsers**: Deprecated the `/firmusers` endpoints and introduced `/firm/users` which behaves in the same way.
</Update>
...
- **Events**: `organizer` field has been removed as it is no longer required.
- **Files**: File extension renames are now supported using the PATCH endpoint.
- **Invoices**: `surcharge` field added to invoices that includes invoice surcharge type and amounts.
- **Staff**: `outOfOffice` field added to staff members, this field contains `startDate` and `endDate`.
- **Users**: Users can now be registered using Authorization Code Grant using the `users/write` scope.
</Update>
...
- **BillingConfiguration**: Added `BillingConfiguration` [GET](/api-reference/matters/get-matter-billing-configuration) and [PUT](/api-reference/matters/update-matter-billing-configuration) endpoints. Billing configurations are set on the matter level.
...
- **Files**: Added [PATCH](/api-reference/files/patch-a-file) endpoint
...
- **Webhooks**: Added `error` event type. This can be used to listen to errors when your API requests fail, see [Error Handling](/docs/api-docs/wivbkstcwngb5-webhooks#error-handling).
</Update>

<Update label="September 2022">
- **Contacts**: Added ability to exclude deleted contacts from GET response (see `ExcludeDeletedContacts` [query parameter](/api-reference/contacts/get-contacts)).
...
- **Matters**: Added ability to filter matters by contact id (see `ContactId` [query parameter](/api-reference/matters/get-matters)).
- **Matters**: Matter access can now be restricted by `UserId` when using Client Credentials grant (see [Acting on behalf of a user](/docs/api-docs/ivmfjbc61tvjs-making-requests#acting-on-behalf-of-a-user)).
- **Leads**: Restricted leads API to Grow and Prosper tiered clients.
- **Files**: File access can now be restricted by `UserId` when using Client Credentials grant (see [Acting on behalf of a user](/docs/api-docs/ivmfjbc61tvjs-making-requests#acting-on-behalf-of-a-user)).
</Update>

<Update label="July 2022">
- **Activities**: Fix `TaxInclusive` field returning incorrect value.
- **Files**: Added ability to add additional data on creation of a file (see [Files](/api-reference/files/add-file-to-a-matter)).
- **Roles**: Fix some [DELETE](/api-reference/roles/remove-role-from-a-matter) endpoint issues.
- **Relationships**: Added [DELETE](/api-reference/relationships/remove-relationship-from-a-role-in-a-matter) relationship endpoint.
- **Tasks**: Added ability to link files and memos to tasks (see [TaskDocuments](/api-reference/tasks/get-task-documents)).
