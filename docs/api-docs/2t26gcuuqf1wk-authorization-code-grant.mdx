...
<CodeGroup>
  ```bash curl
  curl -X GET 'https://auth.smokeball.com/oauth2/authorize?response_type=code&client_id=xxxxx&redirect_uri=https://xxxxxxxx&scope=contacts%2Fread%20matters%2Fread%20matters%2Fwrite'
  ```
  ```json http
  {
    "method": "GET",
    ...
    "query": {
      ...
      "scope": "contacts/read matters/read matters/write"
    }
  }
  ```
</CodeGroup>
...
<ParamField path="scope" type="string" required>
  The [scopes](/docs/api-docs/qf83nz2scopes-scopes) of access being requested, as a space-delimited list (for example `contacts/read matters/read matters/write`).
  Every scope requested must be assigned to your app in the Developer Console.

  OAuth 2.0 expects a client to state the scopes it wants, so always include this parameter.
  The install URL shown on your app's page in the Developer Console already includes every scope assigned to the app, sorted alphabetically, and updates as scopes are added or removed.
  If the parameter is omitted the authorization server currently falls back to issuing a token with all scopes assigned to the app, but you should not rely on this behaviour.
</ParamField>
...
<ResponseField name="refresh_token" type="string">
  ...
  By default, the returned **refresh_token** is valid for 180 days. If you require a longer expiration period, please contact us to discuss your specific needs.

  <Note>
  The 180-day expiry took effect on **24 August 2026** and applies to refresh tokens issued on or after that date.
  Refresh tokens issued before then remain valid for 30 days.
  </Note>
</ResponseField>
...
For further information, see [migration section](#migrating-from-id-token-to-access-token) below for migrating from **id_token** to **access_token**.
...
<CodeGroup>
  ```bash curl
  curl --request GET \
    --url "https://auth.smokeball.com/oauth2/authorize?response_type=code&client_id=xxxxx&redirect_uri=https://your_redirect_uri&scope=contacts%2Fread%20matters%2Fread%20matters%2Fwrite&code_challenge_method=S256&code_challenge=your_code_challenge"
  ```
  ```json http
  {
    "method": "GET",
...
> Note: When you transition to **access_token**, the `scope` query parameter should list only the API scopes your app needs (for example `matters/read`). Please **remember to remove** `openid` from the `scope` parameter in your requests to `https://auth.smokeball.com/oauth2/authorize`, to ensure seamless operation. 