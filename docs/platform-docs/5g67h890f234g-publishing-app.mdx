...
Choose an appropriate name and description for your app before submitting it for review. These help users understand what your app does and help our team assess whether the listing accurately reflects its functionality.

When preparing your app details:

- Use a clear, descriptive app name that reflects your app's purpose
- Avoid vague, overly generic, or misleading names
- Write a concise description that explains the value of your app and the problem it solves
- Make sure the description matches the app's actual functionality and supported workflows
- Review your wording for spelling, grammar, and clarity before publishing
...
- Avoid repeatedly querying our APIs at frequent intervals. Instead, leverage [webhooks](/docs/api-docs/wivbkstcwngb5-webhooks) to be notified when changes occur.
- Polling should only be used when absolutely necessary, and with appropriate intervals.

#### Webhook Usage
- Maintain a single webhook subscription per firm. A subscription can include all the event types you need, and multiple subscriptions lead to duplicate notifications and unnecessary load. See [Maintaining a single subscription](/docs/api-docs/wivbkstcwngb5-webhooks#maintaining-a-single-subscription).
- Check for existing subscriptions after a user re-authenticates and update them rather than creating new ones.
...
- Cache Client Credentials access tokens and reuse them until they expire rather than requesting a new token for every API request. See [Caching Access Tokens](/docs/api-docs/h72024p76i8e1-client-credentials-grant#caching-access-tokens).
...
## Review Process
When your app has been thoroughly tested and is ready for production deployment, you can initiate the review process through the Developer Console. 
Navigate to your app settings and click the "Publish" button to submit your app for review. Before submitting, ensure all your changes have been saved. Any changes you make after submitting will be reflected in the review process.

<Frame>
  <img src="/images/apps/publishing-an-app.png"/>
</Frame>

This will notify our team to begin the review process.
